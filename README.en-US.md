...
- `model`, `service_tier`, `effort`: default turn settings. `/model` can switch the default model at runtime from Codex's local visible model cache. `/effort` discovers and validates the selected model's advertised reasoning levels from the same cache; the default config uses `xhigh`. `service_tier` defaults to `standard`; use `fast` only when the connected account supports it.
